...
def analyze_project_data(project_path, num_extreme_words, book_filter_list=None):
    ...
    try:
        ...
        collected_files_for_processing = [] # Use a temporary list to gather all potential files
        ...
        if book_filter_list and settings and hasattr(settings, 'get_book_file_name'):
            for book_id_from_filter in book_filter_list:
                ...
                try:
                    ...
                    if expected_file_path.exists():
                        collected_files_for_processing.append(expected_file_path)
            ...
            for pattern in usfm_file_patterns:
                collected_files_for_processing.extend(project_path_obj.glob(pattern))

        # Deduplicate the collected list of file paths
        if collected_files_for_processing:
            # Convert to set to remove duplicates, then back to list and sort for consistent processing order
            files_to_process = sorted(list(set(collected_files_for_processing)))
        else:
            files_to_process = []

        # Check if, after deduplication, there are still files to process
        ...
        if SIL_MACHINE_AVAILABLE and processed_book_ids:
            if not settings: # Check if settings object is valid before proceeding
                warning_msg_no_settings = f"Verse counting skipped for {project_name}: Paratext project settings could not be loaded. "
                # print(warning_msg_no_settings) # For immediate feedback if needed
                if "Warning" not in project_results["ProcessingStatus"] and "Error" not in project_results["ProcessingStatus"]:
                    project_results["ProcessingStatus"] = "Warning"
                project_results["ErrorMessage"] += warning_msg_no_settings
            else: # Settings object exists, proceed with corpus attempt
                try:
                    ...
                    for book_id_for_verses in processed_book_ids:
                        ...
                        try:
                            ...
                            # else: text_obj is None (book not found in corpus), no error message needed here specifically for verse counting
                                        # Store unique verse references (book_num, chapter_num, verse_num)
                                        book_verse_counts_detail[book_id_for_verses].add(
                                            (segment.ref.book_num, segment.ref.chapter_num, segment.ref.verse_num)
                                        )
                            # else: # text_obj is None, means book not found in corpus or empty
                            #     msg_no_text = f"No text segments found for book {book_id_for_verses} in {project_name}. "
                            #     if "Warning" not in project_results["ProcessingStatus"] and "Error" not in project_results["ProcessingStatus"]:
                            #         project_results["ProcessingStatus"] = "Warning"
                            #     project_results["ErrorMessage"] += msg_no_text
                            ...
                            if "Warning" not in project_results["ProcessingStatus"] and "Error" not in project_results["ProcessingStatus"]:
                                project_results["ProcessingStatus"] = "Warning"
                            project_results["ErrorMessage"] += msg_corpus_text_err
                    
                    if not book_verse_counts_detail and processed_book_ids:
                        # This means we iterated books but found no verses.
                        msg_no_verses_found = f"No verse references collected for any processed books in {project_name}. BookStats will be empty. Possible reasons: corpus empty, segments have no verse refs, or book text processing issues. "
                        # print(msg_no_verses_found)
                        if "Warning" not in project_results["ProcessingStatus"] and "Error" not in project_results["ProcessingStatus"]:
                             project_results["ProcessingStatus"] = "Warning"
                        project_results["ErrorMessage"] += msg_no_verses_found
                    ...
                    msg_corpus_init_err = f"Could not initialize ParatextTextCorpus for verse counting in {project_name}: {e_corpus_init}. BookStats will be empty. "
                    # print(msg_corpus_init_err) # For immediate feedback
                    if "Warning" not in project_results["ProcessingStatus"] and "Error" not in project_results["ProcessingStatus"]:
                        project_results["ProcessingStatus"] = "Warning"
                    project_results["ErrorMessage"] += msg_corpus_init_err
        elif SIL_MACHINE_AVAILABLE and not processed_book_ids: # SIL_MACHINE is available, but no books were identified to process
            msg_no_books_for_vc = f"Verse counting skipped for {project_name}: No books were processed or identified prior to verse counting stage. "
            # print(msg_no_books_for_vc)
            if "Warning" not in project_results["ProcessingStatus"] and "Error" not in project_results["ProcessingStatus"]:
                project_results["ProcessingStatus"] = "Warning"
            project_results["ErrorMessage"] += msg_no_books_for_vc
        # If SIL_MACHINE_AVAILABLE is False, the global warning is printed at the top of the script, and this block is skipped.
